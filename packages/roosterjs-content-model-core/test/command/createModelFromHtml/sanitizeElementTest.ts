import {
    ...
    DisallowedTags,
    sanitizeElement,
} from '../../../lib/command/createModelFromHtml/sanitizeElement';
...
describe('sanitizeHtml', () => {
    ...
    it('should strip invisible Unicode from href attribute', () => {
        const element = document.createElement('a');
        element.setAttribute('href', 'mailto:\u200Buser@example.com');
        element.textContent = 'Send email';

        const result = sanitizeElement(element, AllowedTags, DisallowedTags);

        expect(result!.getAttribute('href')).toBe('mailto:user@example.com');
        expect(result!.textContent).toBe('Send email');
    });

    it('should strip Unicode Tags from href attribute', () => {
        const element = document.createElement('a');
        // U+E0061 = \uDB40\uDC61 (Tag Latin Small Letter A)
        element.setAttribute('href', 'mailto:\uDB40\uDC61user@example.com');
    ...
    it('should strip bidirectional marks from href attribute', () => {
        const element = document.createElement('a');
        element.setAttribute('href', 'mailto:\u202Auser\u202E@example.com');
        element.textContent = 'Click';
    ...
    it('should not strip invisible Unicode from non-href attributes', () => {
        const element = document.createElement('div');
