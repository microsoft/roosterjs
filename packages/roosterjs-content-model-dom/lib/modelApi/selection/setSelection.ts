...
function setSelectionToSegment(
    ...
    switch (segment.segmentType) {
        ...
        case 'Image':
            ...
            if (
                needToSetSelection(segment, isInSelection) ||
                !segment.isSelectedAsImageSelection != !isSelectedAsImageSelection
            ) {
                const [_, mutableSegments] = mutateParagraph(paragraph, [segment]);

                if (mutableSegments.length > 0) {
                    const mutableImage = mutableSegments[0] as ContentModelImage;

                    setIsSelected(mutableImage, isInSelection);
...
function internalSetSelectionToSegment<T extends ReadonlyContentModelSegment>(
    ...
    if (needToSetSelection(segment, isInSelection)) {
        const [_, mutableSegments] = mutateParagraph(paragraph, [segment]);

        if (mutableSegments.length > 0) {
            const mutableSegment = mutableSegments[0];
