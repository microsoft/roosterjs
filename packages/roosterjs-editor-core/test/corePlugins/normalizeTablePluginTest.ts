...
describe('NormalizeTablePlugin', () => {
    ...
    describe('Normalize Blockquote |', () => {
        function runTest(input: CreateElementData, expectCallback: (el: HTMLElement) => void) {
            const blockquote = createElement(input, document);

            editor.queryElements = () => [blockquote];
            plugin.onPluginEvent({
                eventType: PluginEventType.EditorReady,
            });

            expectCallback(blockquote as HTMLElement);
        }
        it('Normalize centered blockquote', () => {
            runTest(
                ...
                el => {
                    expect(el.style.marginInlineEnd).toEqual('auto');
                    expect(el.style.marginInlineStart).toEqual('');
                }
            );
        });
        ...
        it('Normalize centered blockquote, RTL', () => {
            runTest(
                ...
                el => {
                    expect(el.style.marginInlineEnd).toEqual('');
                    expect(el.style.marginInlineStart).toEqual('auto');
                }
            );
        });
