...
export const isOneNoteDesktopDocument: GetSourceFunction = props => {
    ...
    const rawHtmlContainsOneNoteAttribute =
        !!environment.isSafari && htmlHeadString.indexOf(ONE_NOTE_ATTRIBUTE_VALUE) > -1;

    // The presence of this attribute confirms its origin from OneNote Desktop
    return (
        htmlAttributes[PastePropertyNames.PROG_ID_NAME] == ONE_NOTE_ATTRIBUTE_VALUE ||
        rawHtmlContainsOneNoteAttribute
    );
};
