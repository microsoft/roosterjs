...
function output(filename, library, queue, execTsc) {
...
function main(config) {
    ...
    for (var i = 0; i < include.length; i++) {
        ...
        for (var j = 0; j < queue.length; j++) {
            process(baseDir, queue, j);
        }

        var commonJsOutputName = (config.output[i] || library) + '.d.ts';
        var amdOutputName = (config.output[i] || library) + '-amd.d.ts';

        // We only need to compile rooster.d.ts, the other output files are only for documentation purpose
        if(include[i] == 'roosterjs/lib/index.d.ts') {
            output(commonJsOutputName, library, queue, true);
            output(amdOutputName, null, queue, true);
        } else {
            output(commonJsOutputName, null, queue, false);
        }
...
main({
    ...
    library: 'roosterjs',
    output: [
        path.resolve(targetDir, 'rooster'),
        path.resolve(targetDir, 'roosterjs-editor-types'),
        path.resolve(targetDir, 'roosterjs-editor-dom'),
        path.resolve(targetDir, 'roosterjs-editor-api'),
        path.resolve(targetDir, 'roosterjs-editor-plugins'),
        path.resolve(targetDir, 'roosterjs-plugin-image-resize'),
        path.resolve(targetDir, 'roosterjs-editor-core'),
    ],
    include: [
        'roosterjs/lib/index.d.ts',
        'roosterjs-editor-types/lib/index.d.ts',
        'roosterjs-editor-dom/lib/index.d.ts',
        'roosterjs-editor-api/lib/index.d.ts',
        'roosterjs-editor-plugins/lib/index.d.ts',
        'roosterjs-plugin-image-resize/lib/index.d.ts',
        'roosterjs-editor-core/lib/index.d.ts',
    ]
});
