...
import { getPendingFormat, setPendingFormat } from '../utils/pendingFormat';
import { ICoreEditor } from '../publicTypes/editor/ICoreEditor';
...
export function formatWithContentModel(
    ...
    if (formatter(model, context)) {
        ...
        const eventData: PluginEventData<'contentChanged'> = {
            contentModel: model,
            selection: selection,
            source: changeSource || ChangeSource.Format,
            changeData: {
                formatApiName: apiName,
                additionalData: getChangeData?.(),
            },
        };
...
function handleNewEntities(editor: ICoreEditor, context: FormatWithContentModelContext) {
    ...
    if (editor.isDarkMode()) {
        context.newEntities.forEach(entity => {
            editor.transformColor(entity.wrapper, true /*toDark*/);
        });
    }
}
