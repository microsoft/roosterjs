...
export function keyboardEnter(editor: IContentModelEditor, rawEvent: KeyboardEvent) {
    ...
    if (shouldEnterWithContentModel(range, rawEvent)) {
        editor.formatContentModel(
            (model, context) => {
                const result = deleteSelection(model, [handleEnterKey], context).deleteResult;
...
//                    |                           |               |                             |
//                   [4]                         [3]             [2]                           [1]
const handleEnterKey: EditingStep = {
    shouldRun: () => true,
    callback: context => {
        const { insertPoint, formatContext, deleteResult } = context;
        ...
        switch (containerBlock?.blockGroupType) {
            case 'ListItem':
                if (
                    ...
                    containerBlock.levels.length > 0
                ) {
                    containerBlock.levels.pop();
                } else {
